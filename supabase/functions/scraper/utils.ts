...
export function isWithinTimeWindow(utcDateStr: string): boolean {
...
export function buildMarkdownList(entries: any[]): string {
  ...
  for (const [date, slots] of byDate.entries()) {
    ...
    for (const slot of slots) {
      const sortedDurations = slot.durations.sort((a: number, b: number) => a - b).join(', ');
      text += `  - ${slot.courtName}: ${slot.timeStr} (${sortedDurations} Min)\n`;
    }
  }
  return text.trim();
}